...
//https://developer.zarinpal.com/webapps/developer/docs/classic/api/errorcodes/

defined('_JEXEC') or die('Restricted access');

class zarinpalHelperzarinpal {
	...
	function __construct ($method, $zarinpalPlugin) {
		$session = JFactory::getSession();
		$this->context = $session->getId();
		$this->_method = $method;
		$this->zarinpalPlugin = $zarinpalPlugin;
		//Set the vendor
		...
		$this->getzarinpalPaymentCurrency();
	}

	function getzarinpalPaymentCurrency ($getCurrency = FALSE) {
	...
	public function loadCustomerData () {
		$this->customerData = new zarinpalHelperCustomerData();
	...
	public function getRequest () {
		$this->debugLog($this->requestData, 'zarinpal ' . $this->requestData['METHOD'] . ' Request variables ', 'debug');
	...
	protected function sendRequest ($post_data) {
		...
		$this->debugLog($post_data, 'zarinpal ' . $post_data['METHOD'] . ' Request variables:', $level);
		$this->debugLog($this->response, 'zarinpal response:', $level);
	...
	public function getSSLCertificate (&$certifPath, &$passPhrase) {
		...
		if ($safePath) {
			$sslCertifFolder = $safePath . "zarinpal";
	...
	protected function _getzarinpalUrl ($protocol = 'https://', $includePath = true) {
		$url = ($this->_method->sandbox) ? $protocol . 'www.sandbox.zarinpal.com' : $protocol . 'www.zarinpal.com';
		if ($includePath) {
			$url .= '/cgi-bin/webscr';
		}
		return $url;
	...
	protected function _getApiUrl () {
		...
		//return ($this->_method->sandbox=='sandbox') ? 'https://api-3t.sandbox.zarinpal.com/nvp' : 'https://api-3t.zarinpal.com/nvp';
		return 'https://api' . $url_auth . $url_environment . '.zarinpal.com/nvp';
	}
	...
	public function displayExtraPluginInfo () {
		...
		if ($this->_method->payment_type == '_xclick-auto-billing' && $this->customerData->getVar('autobilling_max_amount')) {
			...
			$extraInfo .= '<br/>';
			$extraInfo .= vmText::_('VMPAYMENT_zarinpal_PAYMENT_BILLING_MAX_AMOUNT') . ': ' . $cd->priceDisplay($this->customerData->getVar('autobilling_max_amount'));
		}
...
//			$recurringDesc .= vmText::_('VMPAYMENT_zarinpal_TRIAL_PERIODS') . $this->_method->trial1_duration . ': '.$this->_method->trial1_price.'<br />';
//		}
//		$recurringDesc .= vmText::_('VMPAYMENT_zarinpal_SUBSCRIPTION_DURATION').': '.$this->_method->subscription_duration . '<br />';
//		$recurringDesc .= vmText::_('VMPAYMENT_zarinpal_SUBSCRIPTION_TERM').': '.$this->_method->subscription_term . '<br />';
		...
		$recurringDesc = vmText::sprintf('VMPAYMENT_zarinpal_SUBSCRIPTION_DESCRIPTION', $durationValue, $durationUnit, $this->_method->subscription_term);
...
//		$recurringDesc .= vmText::_('VMPAYMENT_zarinpal_SUBSCRIPTION_TERM').': '.$this->_method->subscription_term . '<br />';
		...
		$recurringDesc = vmText::sprintf('VMPAYMENT_zarinpal_PAYMENT_PLAN_DESCRIPTION', $this->_method->payment_plan_term, $durationValue, $durationUnit);
		if ($this->_method->payment_plan_defer && $this->_method->zarinpalproduct == 'std') {
			...
			$recurringDesc .= '<br/>' . vmText::sprintf('VMPAYMENT_zarinpal_PAYMENT_PLAN_INITIAL_PAYMENT', JHTML::_('date', $startDate->toFormat(), vmText::_('DATE_FORMAT_LC4')));
		} else {
			if ($this->_method->payment_plan_defer_strtotime) {
				$startDate = JFactory::getDate($this->_method->payment_plan_defer_strtotime);
				$recurringDesc .= '<br/>' . vmText::sprintf('VMPAYMENT_zarinpal_PAYMENT_PLAN_INITIAL_PAYMENT', JHTML::_('date', $startDate->toFormat(), vmText::_('DATE_FORMAT_LC4')));
				//$recurringDesc .= '<br/>'.vmText::sprintf('VMPAYMENT_zarinpal_PAYMENT_PLAN_INITIAL_PAYMENT',date(vmText::_('DATE_FORMAT_LC4'),strtotime('first day of next month')));
			}
		}
		return $recurringDesc;
	}
	...
	/********************************/
	public function processIPN ($zarinpal_data, $payments) {

		// check that the remote IP is from zarinpal.
		if (!$this->checkzarinpalIps($zarinpal_data)) {
			return false;
		}
		// Validate the IPN content upon zarinpal
		if (!$this->validateIpnContent($zarinpal_data)) {
			return false;
		}
		//Check the zarinpal response
		/*
		 * https://cms.zarinpal.com/us/cgi-bin/?cmd=_render-content&content_ID=developer/e_howto_html_IPNandPDTVariables
		...
		if ($zarinpal_data['txn_type'] == 'subscr_cancel') {
			$order_history['order_status'] = $this->_method->status_canceled;
		} elseif ($zarinpal_data['txn_type'] == 'mp_cancel') {
			$order_history['order_status'] = $this->_method->status_canceled;
		} elseif ($zarinpal_data['txn_type'] == 'subscr_eot') {
			$order_history['order_status'] = $this->_method->status_expired;
		} elseif ($zarinpal_data['txn_type'] == 'recurring_payment_expired') {
			$order_history['order_status'] = $this->_method->status_expired;
		} elseif ($zarinpal_data['txn_type'] == 'subscr_signup') {
			//TODO: Validate the response
			$order_history['order_status'] = $this->_method->status_success;
		} elseif ($zarinpal_data['txn_type'] == 'recurring_payment_profile_created') {
			if ($zarinpal_data['profile_status'] == 'Active') {
				$order_history['order_status'] = $this->_method->status_success;
			} else {
				$order_history['order_status'] = $this->_method->status_canceled;
			}
			...
			if (strcmp($zarinpal_data['payment_status'], 'Completed') == 0) {
				...
				if ($this->_check_txn_id_already_processed($payments, $zarinpal_data['txn_id'])) {
					$this->debugLog($zarinpal_data['txn_id'], '_check_txn_id_already_processed', 'debug');
					return FALSE;
				}
				// 3. check email and amount currency is correct
				if ($zarinpal_data['txn_type'] != 'recurring_payment' && !$this->_check_email_amount_currency($payments, $zarinpal_data)) {
					return FALSE;
				}
				// now we can process the payment
				if (strcmp($zarinpal_data['payment_status'], 'Authorization') == 0) {
					$order_history['order_status'] = $this->_method->status_pending;
				} else {
					$order_history['order_status'] = $this->_method->status_success;
				}
				$order_history['comments'] = vmText::sprintf('VMPAYMENT_zarinpal_PAYMENT_STATUS_CONFIRMED', $this->order['details']['BT']->order_number);

			} elseif (strcmp($zarinpal_data['payment_status'], 'Pending') == 0) {
				$lang = JFactory::getLanguage();
				$key = 'VMPAYMENT_zarinpal_PENDING_REASON_FE_' . strtoupper($zarinpal_data['pending_reason']);
				if (!$lang->hasKey($key)) {
					$key = 'VMPAYMENT_zarinpal_PENDING_REASON_FE_DEFAULT';
				}
				$order_history['comments'] = vmText::sprintf('VMPAYMENT_zarinpal_PAYMENT_STATUS_PENDING', $this->order['details']['BT']->order_number) . vmText::_($key);
				$order_history['order_status'] = $this->_method->status_pending;

			} elseif (strcmp($zarinpal_data['payment_status'], 'Refunded') == 0) {
				if ($this->_is_full_refund($payments, $zarinpal_data)) {
					$order_history['comments'] = vmText::sprintf('VMPAYMENT_zarinpal_PAYMENT_STATUS_REFUNDED', $this->order['details']['BT']->order_number);
					$order_history['order_status'] = $this->_method->status_refunded;
				} else {
					$order_history['comments'] = vmText::sprintf('VMPAYMENT_zarinpal_PAYMENT_STATUS_PARTIAL_REFUNDED', $this->order['details']['BT']->order_number);
					$order_history['order_status'] = isset($this->_method->status_partial_refunded) ? $this->_method->status_partial_refunded : 'R';
				}

			} elseif (isset ($zarinpal_data['payment_status'])) {
				// voided
				$order_history['order_status'] = $this->_method->status_canceled;
			} else {
				/*
				* a notification was received that concerns one of the payment (since $zarinpal_data['invoice'] is found in our table),
				* but the IPN notification has no $zarinpal_data['payment_status']
				* We just log the info in the order, and do not change the status, do not notify the customer
				*/
				$order_history['comments'] = vmText::_('VMPAYMENT_zarinpal_IPN_NOTIFICATION_RECEIVED');
	...
	protected function checkzarinpalIps ($zarinpal_data) {
		/*
				$test_ipn = (array_key_exists('test_ipn', $zarinpal_data)) ? $zarinpal_data['test_ipn'] : 0;
				if ($test_ipn == 1) {
					return true;
				}
		*/
		$order_number = $zarinpal_data['invoice'];

		// Get the list of IP addresses for www.zarinpal.com and notify.zarinpal.com


		if ($this->_method->sandbox) {
			$zarinpal_iplist = gethostbynamel('ipn.sandbox.zarinpal.com');
			$zarinpal_iplist = (array)$zarinpal_iplist;
			$this->debugLog($zarinpal_iplist, 'checkzarinpalIps SANDBOX', 'debug', false);

		} else {
			$zarinpal_iplist1 = gethostbynamel('www.zarinpal.com');
			$zarinpal_iplist2 = gethostbynamel('notify.zarinpal.com');
			$zarinpal_iplist3 = array('216.113.188.202', '216.113.188.203', '216.113.188.204', '66.211.170.66');
			$zarinpal_iplist = array_merge($zarinpal_iplist1, $zarinpal_iplist2, $zarinpal_iplist3);
// http://forum.virtuemart.net/index.php?topic=115375.msg406664#msg406664

			// Added JH 2013-10-12
			//Current IP addresses
			//------------api.zarinpal.com---------
			$zarinpal_iplist_api = array(
				'173.0.88.66',
				'173.0.88.98',
				'173.0.84.66',
				'173.0.84.98',
			...
			//------------api-aa.zarinpal.com------------
			$zarinpal_iplist_api_aa = array('173.0.88.67', '173.0.88.99', '173.0.84.99', '173.0.84.67');
			//'------------api-3t.zarinpal.com------------'
			$zarinpal_iplist_api_3t_aa = array('173.0.88.69', '173.0.88.101', '173.0.84.69', '173.0.84.101');
			//------------api-aa-3t.zarinpal.com------------
			$zarinpal_iplist_api_aa_3t = array('173.0.88.68', '173.0.88.100', '173.0.84.68', '173.0.84.100');
			//------------notify.zarinpal.com (IPN delivery)------------
			$zarinpal_iplist_notify = array('173.0.81.1', '173.0.81.33');
			//'-----------reports.zarinpal.com-----------'
			$zarinpal_iplist_reports = array(
				'66.211.168.93',
				'173.0.84.161',
				'173.0.84.198',
				'173.0.88.161',
			...
			//'------------www.zarinpal.com------------'
			//'Starting September 12, 2012 www.zarinpal.com will start resolving to a dynamic list of IP addresses and as such should not be whitelisted.'
			//'For more information on IPNs please go here.'
			//'------------ipnpb.zarinpal.com------------'
			$zarinpal_iplist_ipnb = array(
				'64.4.240.0',
				'64.4.240.1',
				'64.4.240.2',
				'64.4.240.3',
			...
			$zarinpal_iplist = array_merge($zarinpal_iplist, // Added JH 2013-10-12
				$zarinpal_iplist_api, $zarinpal_iplist_api_aa, $zarinpal_iplist_api_3t_aa, $zarinpal_iplist_api_aa_3t, $zarinpal_iplist_notify, $zarinpal_iplist_ipnb// JH
			);

			$this->debugLog($zarinpal_iplist, 'checkzarinpalIps PRODUCTION', 'debug', false);

		}
		$IP=$this->getRemoteIPAddress();
		$this->debugLog($IP, 'checkzarinpalIps REMOTE ADDRESS', 'debug', false);

		//  test if the remote IP connected here is a valid IP address
		if (!in_array($IP, $zarinpal_iplist)) {

			$text = "Error with REMOTE IP ADDRESS = " . $IP . ".
                        The remote address of the script posting to this notify script does not match a valid zarinpal IP address\n
            These are the valid IP Addresses: " . implode(",", $zarinpal_iplist) . "The Order ID received was: " . $order_number;
			$this->debugLog($text, 'checkzarinpalIps', 'error', false);
			return false;
		}
	...
	protected function validateIpnContent ($zarinpal_data) {
		$test_ipn = (array_key_exists('test_ipn', $zarinpal_data)) ? $zarinpal_data['test_ipn'] : 0;
		if ($test_ipn == 1) {
			//return true;
		}
		$zarinpal_data=$_POST;
		// zarinpal wants to open the socket in SSL
		$port = 443;
		$zarinpal_url = $this->_getzarinpalURL('ssl://', false);
		$zarinpal_url_header = $this->_getzarinpalURL('', false);
		$protocol = 'ssl://';
		/*
		 * Before we can trust the contents of the message, we must first verify that the message came from zarinpal.
...
// read the post from zarinpal system and add 'cmd'
		$post_msg = 'cmd=_notify-validate';
		if (function_exists('get_magic_quotes_gpc')) {
			$get_magic_quotes_exists = true;
		}
		foreach ($zarinpal_data as $key => $value) {
		...
		$header .= "Host: " . $zarinpal_url_header . ":" . $port . "\r\n";
		$header .= "Content-Type: application/x-www-form-urlencoded\r\n";
		$header .= "Content-Length: " . strlen($post_msg) . "\r\n";
		//$header .= "Accept: */*\r\n\r\n";
		$header .= "Connection: close\r\n\r\n";

		$fps = fsockopen($zarinpal_url, $port, $errno, $errstr, 30);
		$valid_ipn = false;
		if (!$fps) {
			$this->debugLog(vmText::sprintf('VMPAYMENT_zarinpal_ERROR_POSTING_IPN', $errstr, $errno), 'validateIpnContent', 'error', false);
		} else {
			...
			if (!$valid_ipn) {
				if (strstr($res, "INVALID")) {
					$errorInfo = array("zarinpal_data" => $zarinpal_data, 'post_msg' => $post_msg, 'zarinpal_res' => $res);
					$this->debugLog($errorInfo, vmText::_('VMPAYMENT_zarinpal_ERROR_IPN_VALIDATION'), 'error', false);
				} else {
					$this->debugLog(vmText::_('VMPAYMENT_zarinpal_ERROR_IPN_VALIDATION') . ": NO ANSWER FROM zarinpal", 'validateIpnContent', 'error', false);
				}
			}
		}
	...
	protected function _check_txn_id_already_processed ($payments, $txn_id) {
		...
		if ($this->order['details']['BT']->order_status == $this->_method->status_success) {
			foreach ($payments as $payment) {
				$zarinpal_data = json_decode($payment->zarinpal_fullresponse);
				if ($zarinpal_data->txn_id == $txn_id) {
					return true;
				}
			}
		}
	...
	protected function _check_email_amount_currency ($payments, $zarinpal_data) {
		...
		* If the payment is sent to a non-primary email address on your zarinpal account,
		* the receiver_email is still your primary email.
		*/

		if ($this->_method->zarinpalproduct == "std") {
			if (strcasecmp($zarinpal_data['business'], $this->merchant_email) != 0) {
				$errorInfo = array("zarinpal_data" => $zarinpal_data, 'merchant_email' => $this->merchant_email);
				$this->debugLog($errorInfo, 'IPN notification: wrong merchant_email', 'error', false);
				return false;
			}
		}
		$result = false;
		if ($this->_method->zarinpalproduct == "std" and $zarinpal_data['txn_type'] == 'cart') {
			if (abs($payments[0]->payment_order_total - $zarinpal_data['mc_gross'] < abs($zarinpal_data['mc_gross'] * 0.001)) and ($this->currency_code_3 == $zarinpal_data['mc_currency'])) {
				$result = TRUE;
			}
		} else {
			if (($payments[0]->payment_order_total == $zarinpal_data['mc_gross']) and ($this->currency_code_3 == $zarinpal_data['mc_currency'])) {
				$result = TRUE;
			}
		}
		if (!$result) {
			$errorInfo = array(
				"zarinpal_data"         => $zarinpal_data,
	...
	static function getzarinpalCreditCards () {
		return array(
			'Visa',
			'Mastercard',
			'Amex',
	...
	function  _is_full_refund ($payment, $zarinpal_data) {
		if (($payment->payment_order_total == (-1 * $zarinpal_data['mc_gross']))) {
			return TRUE;
		} else {
			return FALSE;
		}
	...
	function onShowOrderBEPayment ($data) {
		...
		if (isset($data->ACK)) {
			if ($data->ACK == 'SuccessWithWarning' && $data->L_ERRORCODE0 == self::FMF_PENDED_ERROR_CODE && $data->PAYMENTSTATUS == "Pending"
				...
				$html .= $this->zarinpalPlugin->getHtmlRowBE($prefix . $showOrderField, $this->highlight($data->$showOrderField));
			}
			if (($data->ACK == 'Failure' OR $data->ACK == 'FailureWithWarning')) {
				$showOrderField = 'L_SHORTMESSAGE0';
				$html .= $this->zarinpalPlugin->getHtmlRowBE($prefix . 'ERRORMSG', $this->highlight($data->$showOrderField));
		...
		foreach ($showOrderBEFields as $key => $showOrderBEField) {
			if (($showOrderBEField == 'PAYMENTINFO_0_REASONCODE' and isset( $data->$showOrderBEField) and $data->$showOrderBEField != 'None') OR
				...
				if (isset($data->$showOrderBEField)) {
					$key = $prefix . $key;
					$html .= $this->zarinpalPlugin->getHtmlRowBE($key, $data->$showOrderBEField);
				}
			}
	...
	public function debugLog ($message, $title = '', $type = 'message', $echo = false, $doVmDebug = false) {
		...
		$this->zarinpalPlugin->debugLog($message, $title, $type, $doVmDebug);
	}
