<?php
/**
 * zarinpal  Hosted Pro payment plugin.
 *
...
// https://cms.zarinpal.com/cms_content/GB/en_GB/files/developer/HostedSolution.pdf

class zarinpalHelperzarinpalHosted extends zarinpalHelperzarinpal
{
    // Pay Now button; since version 65.1
    const BM_BUTTON_TYPE = 'PAYMENT';
    //A secure button, not stored on zarinpal, used only to initiate the Hosted Solution checkout flow;
    //default for Pay Now button. Since version 65.1
    const BM_BUTTON_CODE = 'TOKEN';
    const BM_BUTTON_VERSION = '104.0';
    const zarinpal_USER_LG = 64;
    public $api_login_id = '';
    ...
    public function __construct($method, $zarinpalPlugin)
    {
        ...
        // if ((empty ($this->_method->payflow_partner) OR empty($this->_method->sandbox_payflow_partner))) {
            ...
            // if ($this->_method->sandbox  ) {
                // $sandbox = 'SANDBOX_';
            // }
            // $text = vmText::sprintf('VMPAYMENT_zarinpal_PARAMETER_REQUIRED', vmText::_('VMPAYMENT_zarinpal_' . $sandbox . 'PAYFLOW_PARTNER'), $this->_method->payment_name, $this->_method->virtuemart_paymentmethod_id);
            // vmError($text);
        // }
    }
    ...
    // todo check the zarinpal langauge: can it be sent. Atm sent in the country lanaguge
    // verfiez la langue, à cause accent
    public function initPostVariables($zarinpalMethod)
    {
        $post_variables = [];
        $post_variables['METHOD'] = $zarinpalMethod;
        $post_variables['VERSION'] = self::BM_BUTTON_VERSION; //https://developer.zarinpal.com/webapps/developer/docs/classic/release-notes/
        ...
        $post_variables['BUTTONIMAGEURL'] = 'https://www.zarinpal.com/en_US/i/btn/btn_paynow_SM.gif'; //we automatically redirect to zarinpal
    ...
    public function addPaymentPageParams(&$post_variables)
    {
    ...
    public function addOrderInfos(&$post_variables)
    {
    ...
    public function addAmount(&$post_variables)
    {
    ...
    public function addUrls(&$post_variables)
    {
        $post_variables['L_BUTTONVAR']['return'] = JURI::root().'index.php?option=com_virtuemart&view=pluginresponse&task=pluginresponsereceived&zarinpalproduct=hosted&on='.$this->order['details']['BT']->order_number.'&pm='.$this->order['details']['BT']->virtuemart_paymentmethod_id.'&Itemid='.vRequest::getInt('Itemid').'&lang='.vRequest::getCmd('lang', '');
        ...
        $post_variables['L_BUTTONVAR']['cancel_return'] = JURI::root().'index.php?option=com_virtuemart&view=pluginresponse&task=pluginUserPaymentCancel&zarinpalproduct=hosted&on='.$this->order['details']['BT']->order_number.'&pm='.$this->order['details']['BT']->virtuemart_paymentmethod_id.'&Itemid='.vRequest::getInt('Itemid').'&lang='.vRequest::getCmd('lang', '');
    }

    public function addConfigPaymentParams(&$post_variables)
    {
    ...
    public function redirectTozarinpal()
    {
        ...
        if ($this->_method->debug and $this->_method->template != 'templateD') {
            echo '<div style="background-color:red;color:white;padding:10px;">The method is in debug mode. <a href="'.$emailink.'">Click here to be redirected to zarinpal</a></div>';
            echo '<div style="background-color:red;color:white;padding:10px;">The method is in debug mode. '.$websitecode.'Click here to be redirected to zarinpal</a></div>';
            jexit();
        } else {
    ...
    public function DoCapture($payment)
    {
        ...
        if ($zarinpal_data->payment_status != 'Pending' && $zarinpal_data->pending_reason != 'Authorization') {
            return false;
        }
        $post_variables = $this->initPostVariables('DoCapture');

        //Do we need to reauthorize ?
        $reauth = $this->doReauthorize($payment->zarinpal_response_txn_id, $zarinpal_data);

        // the authorisation identification number of the payment you want to capture.
        if ($reauth === false) {
            $post_variables['AUTHORIZATIONID'] = $zarinpal_data->txn_id;
        } else {
...
// (Optional) Your invoice number or other identification number that is displayed to the merchant and customer in his transaction history.
    ...
    /**
     * https://developer.zarinpal.com/webapps/developer/docs/classic/zarinpal-payments-standard/integration-guide/authcapture/.
     *
    ...
    public function doReauthorize($AuthorizationID, $zarinpal_data)
    {
        ...
        $post_variables['PAYERID'] = $zarinpal_data->payer_id; // Unique zarinpal customer account identification number
        $post_variables['AUTHORIZATIONID'] = $AuthorizationID;
        $post_variables['PAYMENTACTION'] = 'DoReauthorization';
        $post_variables['AMT'] =  $zarinpal_data->mc_gross; // ???
        $post_variables['CURRENCYCODE'] = $zarinpal_data->mc_currency;
    ...
    {
        $zarinpal_data = json_decode($payment->zarinpal_fullresponse);
        if (strcasecmp($zarinpal_data->payment_status, 'Completed') == 0) {
            $post_variables = $this->initPostVariables('RefundTransaction');
            $post_variables['REFUNDTYPE'] = 'Full';
            $post_variables['TRANSACTIONID'] = $zarinpal_data->txn_id;
        } elseif (strcasecmp($zarinpal_data->payment_status, 'Pending') == 0 && strcasecmp($zarinpal_data->pending_reason, 'authorization') == 0) {
