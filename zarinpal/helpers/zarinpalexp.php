...
//API Reference
//https://developer.zarinpal.com/webapps/developer/docs/classic/api/
// http://www.zarinpalobjects.com/en_US/ebook/PP_ExpressCheckout_IntegrationGuide/HowExpressCheckoutWorks.html#1107932
// http://www.zarinpalobjects.com/en_US/ebook/PP_ExpressCheckout_IntegrationGuide/toc.html

class zarinpalHelperzarinpalExp extends zarinpalHelperzarinpal {
	...
	function __construct ($method, $zarinpalPlugin) {
		parent::__construct($method, $zarinpalPlugin);
		//Set the credentials
		...
		// if ((!$this->ExpCredentialsValid() OR !$this->isAacceleratedOnboardingValid())) {
			// $text = vmText::sprintf('VMPAYMENT_zarinpal_CREDENTIALS_NOT_SET', $this->_method->payment_name, $this->_method->virtuemart_paymentmethod_id);
			// vmError($text, $text);
		// }
		// if (empty ($this->_method->expected_maxamount)) {
			// $text = vmText::sprintf('VMPAYMENT_zarinpal_PARAMETER_REQUIRED', vmText::_('VMPAYMENT_zarinpal_EXPECTEDMAXAMOUNT'), $this->_method->payment_name, $this->_method->virtuemart_paymentmethod_id);
			// vmError($text, $text);
		// }
	...
	function initPostVariables ($zarinpalMethod) {
		$post_variables = Array();
		$post_variables['METHOD'] = $zarinpalMethod;
		$post_variables['version'] = "104.0";
		// 104.0 required by zarinpal
		//https://developer.zarinpal.com/webapps/developer/docs/classic/release-notes/
	...
	/**
	 * https://developer.zarinpal.com/webapps/developer/docs/classic/api/merchant/SetExpressCheckout_API_Operation_NVP/
	...
	/*
	 * languages supported according to this https://cms.zarinpal.com/uk/cgi-bin/?cmd=_render-content&content_ID=developer/e_howto_api_ECCustomizing
	 */
	function getLocaleCode () {
		...
		if (isset($explode[1])) {
			$country = $explode[1];
			if (in_array($country, $zarinpalLanguages)) {
				return $country;
			}
		}
		return "GB";
	...
	public function getToken () {
		...
		if ($this->_method->virtuemart_paymentmethod_id==0) {
			$msg='Programming error,zarinpal expresscheckout: virtuemart_paymentmethod_id is 0';
			vmError($msg,$msg);
			return;
		}
		...
		if ($this->_method->payment_type == '_xclick-subscriptions') {
			...
			if ($this->total == 0) {
				$msg='Programming error,zarinpal expresscheckout: total sent is 0';
				vmError($msg,$msg);
				return;
			}
		...
		// It is almost impossible to have the same amount as the one calultaed by zarinpal
	...
	public function getExpressCheckoutDetails () {
		...
		if ($this->handleResponse()) {
			...
			$this->customerData->setVar('zarinpal_response', $this->response);
			$this->customerData->save();
			$this->storeAddresses();
			return true;
		} else {
	...
	public function DoPayment () {
		...
		$this->sendRequest($post_variables);
		// https://developer.zarinpal.com/webapps/developer/docs/classic/express-checkout/ht_ec_fundingfailure10486/
		...
		if (!$responseValid) {
			if ($this->response['L_ERRORCODE0'] == self::FRAUD_FAILURE_ERROR_CODE and $this->_method->payment_action == 'Sale' and $redirect <= 2) {
				$redirect++;
				// redirect buyer to zarinpal
				$this->redirectTozarinpal($post_variables['TOKEN']);
			}
			return false;
		}
		return true;
	...
	public function CreateRecurringPaymentsProfile () {
		//https://developer.zarinpal.com/webapps/developer/docs/classic/direct-payment/ht_dp-recurringPaymentProfile-curl-etc/
		//https://developer.zarinpal.com/webapps/developer/docs/classic/api/merchant/CreateRecurringPaymentsProfile_API_Operation_NVP/
	...
	function ManageRecurringPaymentsProfileStatus ($payment) {
		...
		$post_variables['PROFILEID'] = $zarinpal_data->PROFILEID;
		$post_variables['ACTION'] = 'Cancel';
		$post_variables['TOKEN'] = $zarinpal_data->TOKEN;
		$post_variables['PAYERID'] = $zarinpal_data->payer_id;

		$this->sendRequest($post_variables);
		$this->handleResponse();

		return $this->GetRecurringPaymentsProfileDetails($zarinpal_data->PROFILEID);
	}
	...
	function DoCapture ($payment) {
		...
		if (strcasecmp($zarinpal_data->PAYMENTINFO_0_PAYMENTSTATUS, 'Pending') != 0 && strcasecmp($zarinpal_data->PAYMENTINFO_0_PENDINGREASON, 'Authorization') != 0) {
			return false;
		}
		...
		$reauth = $this->doReauthorize($zarinpal_data->PAYMENTINFO_0_TRANSACTIONID, $zarinpal_data);
		if ($reauth === false) {
			$post_variables['AUTHORIZATIONID'] = $zarinpal_data->PAYMENTINFO_0_TRANSACTIONID;
		} else {
	...
	function doReauthorize ($AuthorizationID, $zarinpal_data) {
		// TODO
		return false;
		...
		$post_variables['AMT'] = $this->total;
		$post_variables['CURRENCYCODE'] = $zarinpal_data->PAYMENTINFO_0_CURRENCYCODE;
	...
	function RefundTransaction ($payment) {
		...
		$zarinpal_data = json_decode($payment->zarinpal_fullresponse);
		if ($payment->zarinpal_response_payment_status == 'Completed') {
			$post_variables = $this->initPostVariables('RefundTransaction');
			$post_variables['REFUNDTYPE'] = 'Full';
		} else {
			if ($payment->zarinpal_response_payment_status == 'Pending' && $payment->zarinpal_response_pending_reason == 'authorization') {
		...
		$post_variables['AuthorizationID'] = $payment->zarinpal_response_txn_id;
		$post_variables['TRANSACTIONID'] = $payment->zarinpal_response_txn_id;
		$post_variables['TOKEN'] = $zarinpal_data->TOKEN;
		$post_variables['PAYERID'] = $zarinpal_data->payer_id;
	 ...
	 * https://developer.zarinpal.com/docs/classic/express-checkout/ht_ec_fundingfailure10486/
	 * @return bool
	 */
	...
	function storeAddresses () {
		...
		$addressST['shipto_address_type_name'] = 'zarinpal Account';
	...
	function redirectTozarinpal ($token = '') {
		$useraction = '';
		...
		if ($browser->isMobile()) {
			$url = $this->_getzarinpalUrl() . '?cmd=_express-checkout-mobile&token=' . $token . $useraction;
		} else {
			$url = $this->_getzarinpalUrl() . '?cmd=_express-checkout&token=' . $token . $useraction;
		}

		if ($this->_method->debug) {
			echo '<div style="background-color:red;color:white;padding:10px;">The method is in debug mode. <a href="' . $url . '">Click here to be redirected to zarinpal</a></div>';
			jexit();
		} else {
			//header('location: ' . $url);
	...
	function validate ($enqueueMessage = true) {
		...
		if (!$this->customerData->getVar('token')) {
			$this->getToken();
			//Code stops here as the getToken method should redirect to zarinpal
	...
	/**
	 * This page returns a 404 https://www.zarinpal.com/mx/cgi-bin/?cmd=xpt/Merchant/merchant/ExpressCheckoutButtonCode-outside
	 *
	 * code form here
	 * https://www.zarinpalobjects.com/IntegrationCenter/ic_express-buttons.html
	 * @return array
	 */
	...
	function getExpressProduct () {
		...
		$lang = jFactory::getLanguage();
		$lang_iso = str_replace('-', '_', $lang->gettag());
		$zarinpal_buttonurls = array(
			'en_US' => 'https://www.zarinpal.com/en_US/i/logo/zarinpal_mark_60x38.gif',
			'en_GB' => 'https://www.zarinpal.com/en_GB/i/bnr/horizontal_solution_PP.gif',
			'de_DE' => 'https://www.zarinpal.com/de_DE/DE/i/logo/lockbox_150x47.gif',
			'es_ES' => 'https://www.zarinpalobjects.com/WEBSCR-600-20100105-1/en_US/FR/i/bnr/bnr_horizontal_solution_PP_327wx80h.gif',
			'pl_PL' => 'https://www.zarinpalobjects.com/WEBSCR-600-20100105-1/en_US/FR/i/bnr/bnr_horizontal_solution_PP_327wx80h.gif',
			'nl_NL' => 'https://www.zarinpalobjects.com/WEBSCR-600-20100105-1/en_US/FR/i/bnr/bnr_horizontal_solution_PP_327wx80h.gif',
			'fr_FR' => 'https://www.zarinpalobjects.com/WEBSCR-600-20100105-1/en_US/FR/i/bnr/bnr_horizontal_solution_PP_327wx80h.gif',
			'it_IT' => 'https://www.zarinpalobjects.com/WEBSCR-600-20100105-1/it_IT/IT/i/bnr/bnr_horizontal_solution_PP_178wx80h.gif',
			'zn_CN' => 'https://www.zarinpalobjects.com/WEBSCR-600-20100105-1/en_US/FR/i/bnr/bnr_horizontal_solution_PP_327wx80h.gif'
		);
		$zarinpal_infolink = array(
			'en_US' => 'https://www.zarinpal.com/us/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIszarinpal-outside',
			'en_GB' => 'https://www.zarinpal.com/uk/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIszarinpal-outside',
			'de_DE' => 'https://www.zarinpal.com/de/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIszarinpal-outside',
			'es_ES' => 'https://www.zarinpal.com/es/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIszarinpal-outside',
			'pl_PL' => 'https://www.zarinpal.com/pl/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIszarinpal-outside',
			'nl_NL' => 'https://www.zarinpal.com/nl/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIszarinpal-outside',
			'fr_FR' => 'https://www.zarinpal.com/fr/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIszarinpal-outside',
			'it_IT' => 'https://www.zarinpal.com/it/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIszarinpal-outside',
			'zn_CN' => 'https://www.zarinpal.com/cn/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIszarinpal-outside'
		);
		if (!isset($zarinpal_buttonurls[$lang_iso])) {
			$lang_iso = 'en_US';
		}
		$zarinpalProduct['link'] = $zarinpal_infolink[$lang_iso];
		$zarinpalProduct['img'] = $zarinpal_buttonurls[$lang_iso];
		return $zarinpalProduct;
